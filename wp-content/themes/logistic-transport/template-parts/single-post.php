...
<article>
	...
	<?php } ?>
	<?php if( get_theme_mod( 'logistic_transport_feature_image',true) != '') { ?>
	...
	<?php if( get_theme_mod( 'logistic_transport_tags',true) != '') { ?>
	...
	if( get_theme_mod( 'logistic_transport_comment',true) != '') {
	...
	if ( is_singular( 'attachment' ) ) {
		...
		the_post_navigation( array(
			'prev_text' => _x( '<span class="meta-nav">Published in</span><span class="post-title m-3">%title</span>', 'Parent post link', 'logistic-transport' ),
		) );
	} elseif ( is_singular( 'post' ) ) {
		if( get_theme_mod( 'logistic_transport_nav_links',true) != '') {
			// Previous/next post navigation.
			the_post_navigation( array(
				'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_html(get_theme_mod('logistic_transport_next_text',__( 'Next Post', 'logistic-transport' ))) . '<i class="fas fa-chevron-right"></i></span> ' .
					'<span class="screen-reader-text">' . __( 'Next Post', 'logistic-transport' ) . '</span> ' .
					'',
				'prev_text' => '<span class="meta-nav" aria-hidden="true"><i class="fas fa-chevron-left"></i>' . esc_html(get_theme_mod('logistic_transport_prev_text',__( 'Previous Post', 'logistic-transport' ))) . '</span> ' .
...
<?php if (get_theme_mod('logistic_transport_related_posts',true) != '') {
